...
Процедура УдалитьКаталог(Знач ПутьККаталогу, Знач пЛог) Экспорт
	...
	Для ц = 1 По 3 Цикл
		...
		Попытка
			...
			// Некоторые каталоги, например .git, удаляются только так и не с первого раза
			Команда = Новый Команда();
			
			Команда.УстановитьКоманду("rd /s /q");
			Команда.ДобавитьПараметр(ОбернутьВКавычки(ПутьККаталогу));
		...
		Исключение
			пЛог.Предупреждение("Не удалось очистить каталог %1 через rd. Попытка %2", ПутьККаталогу, ц);
		КонецПопытки;
