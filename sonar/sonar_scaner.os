...
Процедура ВыполнитьПроверкуСПомощьюЕДТ()
	...
	имяФайлаРезультата = ВременныеФайлы.СоздатьФайл("tsv");
	ПараметрыПроекта.ExternalIssuesReportPaths = ВременныеФайлы.СоздатьФайл("json");
	рабочаяОбласть = ВременныеФайлы.СоздатьКаталог();
	...
	Команда.ДобавитьПараметр("--project-list " + ОбернутьВКавычки(ПараметрыПроекта.ProjectCatalog));
