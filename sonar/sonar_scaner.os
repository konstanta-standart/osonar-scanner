...
Процедура СонарСканер()
	...
	КомандаСканера.УстановитьКодировкуВывода(КодировкаТекста.OEM);
...
Функция ПерекодироватьИзДос(Знач СтрокаВДос)
	...
	Лог.Отладка(ВремФайл);

	ЗаписьТекста = Новый ЗаписьТекста(ВремФайл, КодировкаТекста.UTF8);
	ЗаписьТекста.ЗаписатьСтроку(СтрокаВДос);
	ЗаписьТекста.Закрыть();

	ВремФайл2 = ВременныеФайлы.СоздатьФайл();
	Лог.Отладка(ВремФайл2);
	
	ЗаписьТекста = Новый ЗаписьТекста(ВремФайл2, КодировкаТекста.OEM);
	ЗаписьТекста.ЗаписатьСтроку(СтрокаВДос);
	ЗаписьТекста.Закрыть();

	ВремФайл3 = ВременныеФайлы.СоздатьФайл();
	Лог.Отладка(ВремФайл3);
	
	ЗаписьТекста = Новый ЗаписьТекста(ВремФайл3, КодировкаТекста.ANSI);
	ЗаписьТекста.ЗаписатьСтроку(СтрокаВДос);
	ЗаписьТекста.Закрыть();

	ВремФайл4 = ВременныеФайлы.СоздатьФайл();
	Лог.Отладка(ВремФайл4);
	
	ЗаписьТекста = Новый ЗаписьТекста(ВремФайл4, КодировкаТекста.Системная);
	ЗаписьТекста.ЗаписатьСтроку(СтрокаВДос);
	ЗаписьТекста.Закрыть();
	
	ЧтениеТекста = Новый ЧтениеТекста(ВремФайл, КодировкаТекста.OEM);
...
Функция ЗапуститьКоманду(Знач Команда) Экспорт
	...
	Процесс = СоздатьПроцесс(строкаЗапуска, ".", Истина, Истина, КодировкаТекста.OEM);
