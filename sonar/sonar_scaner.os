...
Процедура ЗаполнитьПараметрыПроектаИзФайлаНастроек()
	...
	МенеджерПараметров.УстановитьФайлПараметров("./.project-settings.json");
...
Процедура СонарСканер()
	...
	КомандаСканера.УстановитьКоманду("sonar-scanner.bat"); // берем из PATH
	...
	Лог.Информация(КомандаСканера.ПолучитьВывод());
...
Функция ЗапуститьКоманду(Знач Команда) Экспорт
	...
	СтрокаЗапуска = "cmd /c ""@chcp 65001 > nul && " + СтрокаЗапуска + """";

	СтрокаЗапускаДляВывода = СтрЗаменить(СтрокаЗапуска, ПараметрыПроекта.sonar_token, ПредставлениеПараметра(ПараметрыПроекта.sonar_token, Истина));
