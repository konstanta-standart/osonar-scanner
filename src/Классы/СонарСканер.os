...
Процедура ДобавитьВСонарПараметры_ПараметрыПроекта(КомандаСканера)
	...
	КомандаСканера.ДобавитьПараметр("-Dsonar.sources=" + ОбщегоНазначения.ОбернутьВКавычки(ПараметрыПроекта.ПутьКИсходникам()));
...
Процедура ДобавитьВСонарПараметры_ЗапросаНаСлияние(КомандаСканера)
	...
	// Принудительно переопределим параметры пропуска проверки для мерж-реквеста
	// В мерж-реквесте проверяем все изменения - не зависимо от того, на поддержке они или нет.
	КомандаСканера.ДобавитьПараметр("-Dsonar.bsl.languageserver.skipSupport=never");
