...
Процедура ЗаполнитьПараметрИзФайлаНастроек(Знач ИмяПараметра, Знач ЗначениеПараметра, Знач СкрытьЗначение = Ложь)
	...
	Если ПараметрОбъектаЗаполнен(ИмяПараметра) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьПараметрыЗначениямиПоУмолчанию()
	...
	ЗаполнитьПараметрЗначениемПоУмолчанию("sonar_host_url", "");
	...
	путьККорню = ОбъединитьПути(ТекущийСценарий().Каталог, "..", "..", "..");
	путьКФайлуНастроек = ОбъединитьПути(путьККорню, "AddFiles", "settings.json");
	ЗаполнитьПараметрЗначениемПоУмолчанию("ExternalIssuesReportSettings", путьКФайлуНастроек);
